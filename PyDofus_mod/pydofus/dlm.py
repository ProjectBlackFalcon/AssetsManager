...
class Map:
    ...
    def read(self):
        ...
        if self._obj["mapVersion"] >= 9:
            ...
            self._obj["gridColor"] = (grid_alpha & 255) << 32 | (grid_red & 255) << 16 | (
                        grid_green & 255) << 8 | grid_blue & 255
        ...
        self._obj["backgroundColor"] = (self._obj["backgroundRed"] & 255) << 16 | (
                    self._obj["backgroundGreen"] & 255) << 8 | self._obj["backgroundBlue"] & 255
...
class CellData:
    ...
    def read(self):
        ...
        if self.mapVersion > 5:
            ...
            if self.mapVersion > 10 and (self.hasLinkedZoneFight() or self.hasLinkedZoneRp()):
                self.raw().read_uchar()
    ...
    def hasLinkedZoneRp(self):
        if self._obj["mov"] and not self._obj["farmCell"]:
            return True
        else:
            return False

    def hasLinkedZoneFight(self):
        if self._obj["mov"] and not self._obj["nonWalkableDuringFight"] and not self._obj["farmCell"] and self._obj["havenbagCell"]:
            return True
        else:
            return False
